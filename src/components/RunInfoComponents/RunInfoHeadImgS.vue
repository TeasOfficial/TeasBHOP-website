        ...
        <span v-if="!runinfo.loading.value">
          ...
          <v-card-subtitle>
            ...
            <span class="btngroup">
              ...
              &ensp;
              <v-btn v-if="isSourceJump" color="white" :to="`/user/${runinfo.auth.value}?type=sj`" variant="text">
                {{ $t('runinfo.view.userprofile') }}
              </v-btn>

              <v-btn v-else color="white" :to="`/user/${runinfo.auth.value}?type=auth`" variant="text">
                {{ $t('runinfo.view.userprofile') }}
              </v-btn>
              &ensp;
...
<script setup>
  ...
  const isSourceJump = route.query.c.startsWith('SourceJump')
</script>
